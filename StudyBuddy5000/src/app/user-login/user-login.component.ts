...
export class UserLoginComponent implements OnInit {
  ...
  isRegistered(userName: string):userData|undefined {
    if (!this.isUsers()) {
      return undefined;
    }
    return this.users
  ...
  onLogin(form: NgForm) {
    ...
    if(this.loggedInUser as LoggedInUser){
      let loggedIn = this.loggedInUser as LoggedInUser;
        if(loggedIn.User){
          this.api.setUser(loggedIn.User as userData) // passing the currently logged in user back to service so it is globally available, has to be done this way...
          return;
      }
    }
  }
